...
# ⚠️ 注意：板书提示词已迁移到模块化结构（constants_blackboard.py）
# 以下 DEFAULT_BLACKBOARD_PROMPT 引用新的模块化版本，保持向后兼容
#
# 模块化常量：
#   - BLACKBOARD_CORE_RULES: 核心规则（不可变）
#   - BLACKBOARD_LIBRARIES: 库能力说明（不可变）
#   - BLACKBOARD_ACTIONS: Action 使用指南（不可变）
#   - BLACKBOARD_OPTIMIZATION_STRATEGY: 优化策略（可定制）
#   - build_blackboard_prompt(): 构建完整提示词
#
# 使用方法：
#   - 默认提示词: DEFAULT_BLACKBOARD_PROMPT
#   - 部分定制: build_blackboard_prompt(custom_strategy)
#   - 完全定制: 直接设置自定义字符串

# ========== 模块化板书提示词导入 ==========
# 从 constants_blackboard.py 导入新的模块化常量
try:
    ...
    # 使用新的模块化版本构建默认提示词
    DEFAULT_BLACKBOARD_PROMPT = build_blackboard_prompt()
except ImportError:
    # 向后兼容：如果 constants_blackboard.py 不存在，使用简化的默认值
    # 完整版本请参考 constants_blackboard.py
    DEFAULT_BLACKBOARD_PROMPT = """<blackboard_mode_instructions>
你现在处于"板书模式"，请以 JSON 格式输出板书步骤。
每个步骤包含: html（增量 HTML）、narration（讲解文字）、step_number（步骤号）、is_complete（是否完成）。
详细规范请参考 constants_blackboard.py 中的完整提示词。
