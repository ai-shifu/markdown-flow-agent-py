...
Use HTML for text layout. Use SVG only for pure graphics (icons, arrows, connector lines). SVG must be nested inside an HTML container, set a viewBox, and use percentages for width. Text inside SVG is limited to no more than 4 Chinese characters. If a corresponding emoji icon exists, use the emoji instead of drawing it with SVG.
